Extract all rules together with Extensions
    ...
    FROM
    ( SELECT *,
       array_to_string(REGEXP_MATCH(convert_from(crx.rule_data, 'UTF8'), ' owner="([\.\w\-_]+)" '), '') AS crowner,
       array_to_string(REGEXP_MATCH(convert_from(crx.rule_data, 'UTF8'), '><name>(.*)</name><notes>'), '') AS crname,
        CASE WHEN (
